...
def calibrate():
    ...
    coords = _load_coords()
    coords["chest1"] = {"x": x1, "y": y1}
    coords["chest2"] = {"x": x2, "y": y2}
    _save_coords(coords)
...
def auto_click(mean, std, lo, hi, jitter, break_every, session_max, loop_count, transfer_every, transfer_unit):
    coords = _load_coords()
...
def main():
    ...
    parser.add_argument("--break-every", type=int, default=30, help="Nghỉ 3-8 phút sau mỗi N vòng (0 = không nghỉ)")
    parser.add_argument("--session-max", type=float, default=90.0, help="Tự dừng sau X phút (0 = không giới hạn)")
